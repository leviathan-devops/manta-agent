...
export class CompactionManager {
  ...
  private writeExport(trigger: CompactionExport['trigger']): void {
    ...
    // Cap exports to 10 most recent
    try {
      const parentDir = path.dirname(dir);
      const allExports = fs.readdirSync(parentDir)
        .filter((d: string) => d.startsWith('export-'))
        .map((d: string) => ({ name: d, time: fs.statSync(path.join(parentDir, d)).mtimeMs }))
        .sort((a, b) => b.time - a.time);

      if (allExports.length > 10) {
        const toRemove = allExports.slice(10);
        for (const exp of toRemove) {
          fs.rmSync(path.join(parentDir, exp.name), { recursive: true, force: true });
        }
      }
    } catch (e) {
...
`;
  ...
  private renderChangeLog(ts: string): string {
...
`;
  ...
  private renderDebugLog(ts: string): string {
    const errors = this.errorsUnsolved.length > 0
      ? this.errorsUnsolved.map((e: string) => `## ${ts} — error\n- **Desc:** ${e}\n- **Root Cause:** Pending\n- **Fix:** Pending`).join('\n\n')
      : 'No failures recorded.';
    return `# DEBUG_LOG.md\n\n${errors}\n\n---\n*Updated: ${ts}*`;
  }

  private renderPostCompaction(ts: string): string {
    return `# POST-COMPACTION RECOVERY PROMPT

**Gate:** ${this.currentGate}
**Iteration:** ${this.currentIteration}
**Phase:** ${this.tasks.find((t: { status: string }) => t.status === 'in_progress')?.description || 'Awaiting task'}
**Active:** ${this.tasks.filter((t: { status: string }) => t.status === 'in_progress').length}
**Completed:** ${this.tasks.filter((t: { status: string }) => t.status === 'done').length}
