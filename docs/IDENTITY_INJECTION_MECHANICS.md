...
CLEAR+REBUILD was originally ported from SPIDER's identity infrastructure pattern. The core insight — "wipe the array, don't patch it" — comes from SPIDER's system-transform-hook where they discovered that runtime defaults kept winning when they only patched individual entries.

The pattern is now MANTA's own. The comment in the code was rebranded from "SPIDER pattern" to "CLEAR+REBUILD" to reflect this.
